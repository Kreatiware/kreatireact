...
const detectFirstDay = (loc: string): number => {
  try {
    ...
    if (l.weekInfo?.firstDay !== undefined) return l.weekInfo.firstDay % 7;
  } catch {
    /* fallback */
  }
  return 0;
...
export const Calendar = ({
  ...
  const [currentView, setCurrentView] = useState<"days" | "months" | "years">(
    viewProp === "month" ? "months" : viewProp === "year" ? "years" : "days"
  );
  ...
  const selectDate = useCallback(
    (d: Date) => {
      ...
      if (selectionMode === "single") {
        ...
        const fallbackTime =
          (showTime || viewProp === "time") && !prev && !pendingTime
            ? {
                h: new Date().getHours(),
  ...
  const setTime = useCallback(
    (h: number, m: number, s: number) => {
      if (!inline && popoverOpen) {
        ...
        const base =
          selected && !Array.isArray(selected) ? selected : new Date();
        updateValue(
          new Date(base.getFullYear(), base.getMonth(), base.getDate(), h, m, s)
        );
      }
    },
  ...
  const navMonth = useCallback(
    (delta: number) =>
      setViewDate(p => new Date(p.getFullYear(), p.getMonth() + delta, 1)),
    []
  );
  const navYear = useCallback(
    (delta: number) =>
      setViewDate(p => new Date(p.getFullYear() + delta, p.getMonth(), 1)),
    []
  );
  ...
  const monthNames = useMemo(() => {
    ...
    return Array.from({ length: 12 }, (_, i) => {
      const n = fmt.format(new Date(2024, i, 1));
  ...
  const weekdayNames = useMemo(() => {
    ...
    return Array.from({ length: 7 }, (_, i) => {
      ...
      return weekdayFormat === "narrow"
        ? n.charAt(0).toUpperCase()
        : n.charAt(0).toUpperCase() + n.slice(1);
    });
  }, [resolvedLocale, firstDayOfWeek, weekdayFormat]);

  const buildDaysGrid = useCallback(
      ...
      <div className={`${base}__panel`} key={monthOffset}>
        {numberOfMonths === 1 && (
          <div className={`${base}__header`}>
            <button
              ...
              onClick={() => navMonth(-1)}
              aria-label={kreatiLocale.calendar.previousMonth}
            >
              {chevronLeft}
            </button>
            <button
            ...
            <button
              ...
              className={`${base}__nav`}
              onClick={() => navMonth(1)}
              aria-label={kreatiLocale.calendar.nextMonth}
            >
              {chevronRight}
            </button>
          </div>
        ...
        {numberOfMonths > 1 && (
          <div className={`${base}__header`}>
            ...
            <span className={`${base}__title ${base}__title--static`}>
              {monthNames[panelDate.getMonth()]} {panelDate.getFullYear()}
            </span>
            ...
            {monthOffset !== numberOfMonths - 1 && (
              <span className={`${base}__nav-spacer`} />
            )}
          </div>
        )}
        <div className={`${base}__grid`} role="grid">
          <div className={`${base}__weekdays`} role="row">
            {showWeekNumbers && (
              <span className={`${base}__weekday ${base}__wk-header`} />
            )}
            {weekdayNames.map((n, i) => (
              <span key={i} className={`${base}__weekday`} role="columnheader">
                {n}
              </span>
            ))}
          </div>
          {weeks.map((week, wi) => (
            <div key={wi} className={`${base}__week`} role="row">
              {showWeekNumbers && (
                <span className={`${base}__wk`}>{getWeekNumber(week[0])}</span>
              )}
              {week.map(day => {
                ...
                const cls = [
                  `${base}__day`,
                  !unstyledDays && sel && `${base}__day--selected`,
                  !unstyledDays && tod && `${base}__day--today`,
                  out && `${base}__day--outside`,
                  dis && `${base}__day--disabled`,
                  !unstyledDays && inR && `${base}__day--in-range`,
                ]
                  .filter(Boolean)
                  .join(" ");
                return (
    ...
    <>
      <div className={`${base}__header`}>
        <button
          ...
          className={`${base}__nav`}
          onClick={() => navYear(-1)}
          aria-label={kreatiLocale.calendar.previousYear}
        >
          {chevronLeft}
        </button>
        <button
        ...
        <button
          ...
          className={`${base}__nav`}
          onClick={() => navYear(1)}
          aria-label={kreatiLocale.calendar.nextYear}
        >
          {chevronRight}
        </button>
      </div>
      <div className={`${base}__months-grid`} role="grid">
        {shortMonthNames.map((n, i) => (
          <button
            ...
            type="button"
            role="gridcell"
            aria-selected={viewDate.getMonth() === i || undefined}
            className={[
              `${base}__cell`,
              viewDate.getMonth() === i && `${base}__cell--selected`,
            ]
              .filter(Boolean)
              .join(" ")}
            onClick={() => {
              ...
              if (viewProp === "month") {
                updateValue(new Date(viewDate.getFullYear(), i, 1));
                if (!inline) {
                  setPopoverOpen(false);
                  onBlur?.();
                }
        ...
        <div className={`${base}__years-grid`} role="grid">
          {years.map(y => (
            <button
              ...
              type="button"
              role="gridcell"
              aria-selected={viewDate.getFullYear() === y || undefined}
              className={[
                `${base}__cell`,
                viewDate.getFullYear() === y && `${base}__cell--selected`,
              ]
                .filter(Boolean)
                .join(" ")}
              onClick={() => {
                ...
                if (viewProp === "year") {
                  updateValue(new Date(y, 0, 1));
                  if (!inline) {
                    setPopoverOpen(false);
                    onBlur?.();
                  }
                } else setCurrentView("months");
              }}
            >
              {y}
            </button>
  ...
  const renderTimePicker = () => {
    ...
    const onChangeM = (e: React.ChangeEvent<HTMLInputElement>) =>
      applyTime(h, parseInt(e.target.value, 10) || 0, s);
    const onChangeS = (e: React.ChangeEvent<HTMLInputElement>) =>
      applyTime(h, m, parseInt(e.target.value, 10) || 0);

    const onWheelH = (e: React.WheelEvent) => {
      const delta = e.deltaY < 0 ? 1 : -1;
      applyTime(h + delta, m, s);
    };
    const onWheelM = (e: React.WheelEvent) =>
      applyTime(h, m + (e.deltaY < 0 ? 1 : -1), s);
    const onWheelS = (e: React.WheelEvent) =>
      applyTime(h, m, s + (e.deltaY < 0 ? 1 : -1));

    const hourLabel = timeLabelsShort
      ? kreatiLocale.calendar.hourShort
      : kreatiLocale.calendar.hour;
    const minuteLabel = timeLabelsShort
      ? kreatiLocale.calendar.minuteShort
      : kreatiLocale.calendar.minute;
    const secondLabel = timeLabelsShort
      ? kreatiLocale.calendar.secondShort
      : kreatiLocale.calendar.second;

    const isTimeOnly = viewProp === "time";

    return (
      <div
        className={`${base}__time ${isTimeOnly ? `${base}__time--no-border` : ""}`}
      >
        {is12h && (
          <div className={`${base}__time-toggle`}>
            <ToggleButtonGroup
              ...
              onChange={v => {
                const toAM = v === "AM";
                if (toAM && !isAM) setTime(h - 12, m, s);
                else if (!toAM && isAM) setTime(h + 12, m, s);
              }}
              size={size}
            />
          </div>
        ...
        <div className={`${base}__time-fields`} ref={timeFieldsRef}>
          ...
          >
            <Input
              ...
              size={size}
              label={hourLabel}
              value={String(dispH).padStart(2, "0")}
              onChange={onChangeH}
              step={1}
              fullWidth
            />
  ...
  const renderButtonBar = () => {
    ...
    return (
      <div className={`${base}__button-bar`}>
        <Button
          label={kreatiLocale.calendar.today}
          buttonType="text"
          size={size}
        ...
        <Button
          ...
          size={size}
          severity="secondary"
          ariaLabel={kreatiLocale.calendar.clear}
          onClick={handleClear}
        />
      </div>
    );
  ...
  const builtInPresets = useMemo((): CalendarPreset[] => {
    ...
    return [
      ...
      {
        label: kreatiLocale.calendar.presetYesterday,
        value: () => {
          const d = new Date();
      ...
      {
        label: kreatiLocale.calendar.presetLast7Days,
        value: () => {
          const e = new Date();
          const s = new Date();
      ...
      {
        label: kreatiLocale.calendar.presetLast14Days,
        value: () => {
          const e = new Date();
          const s = new Date();
      ...
      {
        label: kreatiLocale.calendar.presetLast30Days,
        value: () => {
          const e = new Date();
          const s = new Date();
      ...
      {
        label: kreatiLocale.calendar.presetThisWeek,
        value: () => [new Date(startOfWeek), new Date()],
      },
      {
        label: kreatiLocale.calendar.presetThisMonth,
        value: () => [new Date(t.getFullYear(), t.getMonth(), 1), new Date()],
      },
      {
        label: kreatiLocale.calendar.presetLastMonth,
        value: () => [
      ...
      {
        label: kreatiLocale.calendar.presetThisYear,
        value: () => [new Date(t.getFullYear(), 0, 1), new Date()],
      },
    ];
  }, [kreatiLocale, firstDayOfWeek]);

  const resolvedPresets =
    presets === true ? builtInPresets : Array.isArray(presets) ? presets : null;

  const presetListItems = useMemo((): ListItem[] | null => {
    if (!resolvedPresets) return null;
    return resolvedPresets.map((p, i) => ({
      key: `preset-${i}`,
      label: p.label,
    }));
  }, [resolvedPresets]);
  ...
  const handlePresetSelect = useCallback(
    (key: string) => {
      ...
      if (Array.isArray(val) && val.length > 0)
        setViewDate(new Date(val[0].getFullYear(), val[0].getMonth(), 1));
      else if (!Array.isArray(val))
        setViewDate(new Date(val.getFullYear(), val.getMonth(), 1));
      if (!inline) {
        setPopoverOpen(false);
        onBlur?.();
      }
  ...
  const calendarPanel = (
    <div
      className={`${base}__body ${base}__body--${size} ${numberOfMonths > 1 ? `${base}__body--multi` : ""} ${viewProp === "time" ? `${base}__body--time-only` : ""} ${viewProp === "time" && showSeconds ? `${base}__body--time-seconds` : ""}`}
    >
  ...
  if (inline) {
    return (
      <div
        ref={wrapperRef}
        className={`${base} ${base}--inline ${className}`.trim()}
        role="group"
        aria-label={kreatiLocale.calendar.calendarLabel}
      >
        {calendarBody}
      </div>
    );
  ...
  const triggerInner = (
    <>
      <span
        className={`${base}__input-text ${!hasValue ? `${base}__input-text--placeholder` : ""}`}
      >
        {hasValue
          ? inputDisplay
          : placeholder || (hasLabel && isFloating ? "\u00A0" : "")}
      </span>
      <span className={`${base}__input-icon`} aria-hidden="true">
        {calendarIcon}
      </span>
    </>
  );
  ...
  const triggerEl = isFloating ? (
    ...
    >
      ...
      {hasLabel && (
        <label
          className={`${base}__floating-label ${hasValue || popoverOpen ? `${base}__floating-label--active` : ""}`}
        >
          {label}
          {required && (
  ...
  const popoverContent = (
    <div
      className={`${base}__popover-content ${touchUI ? `${base}__popover-content--touch` : ""}`}
    >
      {calendarBody}
    </div>
  );
  ...
  const hiddenInput = name ? (
    <input
      ...
      value={
        hasValue
          ? Array.isArray(selected)
            ? selected.map(d => d.toISOString()).join(",")
