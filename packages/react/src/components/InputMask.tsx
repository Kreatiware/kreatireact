...
export interface InputMaskProps extends Omit<InputProps, 'type' | 'value' | 'defaultValue' | 'onChange' | 'maxLength' | 'hideSteppers' | 'decimalSeparator'> {
...
export const InputMask = forwardRef<HTMLInputElement, InputMaskProps>(
  (
    ...
    const extractRaw = useCallback((formatted: string): string => {
      let raw = '';
    ...
    const getMaskPlaceholder = useCallback((): string => {
      return mask.split('').map((ch) => isSlot(ch) ? placeholderChar : ch).join('');
    ...
    const displayValue = isControlled ? applyMask(controlledValue) : internalValue;

    const setCursorAfterFormat = useCallback((el: HTMLInputElement, prevRaw: string, nextFormatted: string) => {
    ...
    }, [applyMask, extractRaw, displayValue, isControlled, onChange, setCursorAfterFormat]);
    ...
    return (
      <Input
        ...
        placeholder={placeholder || (showMaskPlaceholder ? getMaskPlaceholder() : undefined)}
