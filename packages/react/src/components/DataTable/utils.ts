 ...
 */
export const getFieldValue = (obj: Record<string, unknown>, field: string): unknown => {
...
export const matchFilter = (value: unknown, filterValue: unknown, matchMode: FilterMatchMode): boolean => {
...
export const generateCSV = <T extends Record<string, unknown>>(
  ...
  const headers = visibleCols.map(c => `"${(c.header || c.field).replace(/"/g, '""')}"`).join(',');
