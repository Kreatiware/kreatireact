...
export const DataTable = (({
  ...
  style,
  ref,
}: DataTableProps<any> & { ref?: React.Ref<DataTableRef> }) => {
  ...
  const [internalFilters, setInternalFilters] = useState<
    Record<string, ColumnFilterMeta>
  >({});
  ...
  const resolvedColumns = useMemo(() => {
    ...
    if (Object.keys(columnWidths).length > 0) {
      cols = cols.map(col =>
        columnWidths[col.field]
          ? { ...col, width: `${columnWidths[col.field]}px` }
          : col
      );
    }
    ...
    cols = cols.map(col => {
      ...
      if (visibleColumns && !visibleColumns.includes(col.field))
        isHidden = true;
      ...
      if (sortMode === "single") {
        ...
        if (existing >= 0) {
          ...
          if (current.order === "asc") {
            newMeta = multiSortMeta.map((m, i) =>
              i === existing ? { ...m, order: "desc" as SortDirection } : m
            );
          } else {
        ...
        onSort?.(
          field,
          newMeta.find(m => m.field === field)?.order ?? null,
          newMeta
        );
      }
    },
  ...
  const handleFilterApply = useCallback(
    (field: string, value: unknown, matchMode: FilterMatchMode) => {
  ...
  const processedData = useMemo(() => {
    ...
    if (activeGlobalFilter) {
      const fields =
        globalFilterFields || columns.filter(c => !c.hidden).map(c => c.field);
      result = result.filter(row =>
        fields.some(f =>
          matchFilter(getFieldValue(row, f), activeGlobalFilter, "contains")
        )
      );
    }
    ...
    const activeFilters = Object.entries(filters).filter(
      ([, meta]) =>
        meta.value !== "" && meta.value !== null && meta.value !== undefined
    );
    if (activeFilters.length > 0) {
      result = result.filter(row =>
        activeFilters.every(([field, meta]) =>
          matchFilter(getFieldValue(row, field), meta.value, meta.matchMode)
        )
      );
    }
    ...
    if (sortMode === "single" && currentSortField && currentSortOrder) {
      ...
      result.sort((a, b) => {
        if (col?.sortFunction)
          return col.sortFunction(a, b, currentSortField, currentSortOrder);
  ...
  const handleRowSelect = useCallback(
    (
      ...
      if (selectionMode === "single") {
        newSelection = selection.some(s => s[dataKey] === row[dataKey])
          ? []
          : [row];
      } else {
  ...
  const allSelected = useMemo(() => {
    ...
    const selectableRows = paginatedData.filter(
      (r, i) => !disabledRows?.(r, startIndex + i)
    );
    return (
      selectableRows.length > 0 &&
      selectableRows.every(r =>
        selection.some(
          s => s[dataKey] === (r as Record<string, unknown>)[dataKey]
        )
      )
    );
  ...
  const handleSelectAll = useCallback(() => {
    ...
    if (allSelected) {
      const pageKeys = new Set(
        selectableRows.map(r => (r as Record<string, unknown>)[dataKey])
      );
      ...
      const toAdd = selectableRows.filter(
        r => !existing.has((r as Record<string, unknown>)[dataKey])
      );
  ...
  const [frozenLeftOffsets, setFrozenLeftOffsets] = useState<
    Record<string, number>
  >({});
  const [frozenRightOffsets, setFrozenRightOffsets] = useState<
    Record<string, number>
  >({});
  ...
  React.useLayoutEffect(() => {
    ...
    const ths = Array.from(
      headerRow.querySelectorAll<HTMLTableCellElement>(".k-datatable__th")
    );
    ...
    ths.forEach(th => {
      if (
        th.classList.contains("k-datatable__select-cell") ||
        th.classList.contains("k-datatable__expand-cell") ||
        th.classList.contains("k-datatable__drag-cell")
      )
        return;
    ...
    colThs
      .filter(
        c => visibleCols.find(v => v.field === c.field)?.frozen === "left"
      )
      .forEach(({ field, th }) => {
    ...
    [...colThs]
      .reverse()
      .filter(
        c => visibleCols.find(v => v.field === c.field)?.frozen === "right"
      )
      .forEach(({ field, th }) => {
    ...
    (field: string, width: number) => {
      ...
      setColumnWidths(prev => {
        if (Object.keys(prev).length === 0) {
          const table = wrapperRef.current?.querySelector(
            ".k-datatable__table"
          );
          if (table) {
            const ths = table.querySelectorAll<HTMLTableCellElement>(
              ".k-datatable__thead tr:last-child .k-datatable__th"
            );
            const initial: Record<string, number> = {};
            const visibleCols = (internalColumns ?? columnsProp).filter(
              c => !c.hidden
            );
            let colIdx = 0;
            ths.forEach(th => {
              ...
              if (
                ...
                return;
              if (colIdx < visibleCols.length) {
                initial[visibleCols[colIdx].field] = Math.round(
  ...
  const columnGroups = useMemo(() => {
    ...
    columns
      .filter(c => c.columnGroup && !c.hidden)
      .forEach(col => {
  ...
  useImperativeHandle(ref, () => ({
    ...
    copyToClipboard: () => {
      const visibleCols = columns.filter(
        c => !c.hidden && c.field && !c.field.startsWith("__")
      );
  ...
  return (
    <div
      ...
      role="grid"
      aria-busy={loading}
      aria-label={locale.dataTable?.ariaLabel || "Data table"}
    ...
    >
              ...
              <div className="k-datatable__toolbar-actions">
                {toolbar?.includes("copy") && (
                  <Button
                    ...
                    buttonType="outlined"
                    severity="secondary"
                    ariaLabel={locale.dataTable?.copy || "Copy"}
                    tooltip={locale.dataTable?.copy || "Copy"}
                    onClick={() => {
                      const ref = wrapperRef.current;
                      if (ref)
                        (
                          ref as unknown as { __ref?: DataTableRef }
                        ).__ref?.copyToClipboard?.();
                    }}
                  />
                )}
                {toolbar?.includes("export") && (
                  <Button
                    ...
                    buttonType="outlined"
                    severity="secondary"
                    ariaLabel={locale.dataTable?.exportCSV || "Export"}
                    tooltip={locale.dataTable?.exportCSV || "Export"}
                    onClick={() => {
                      const csv = generateCSV(processedData, columns);
                      downloadFile(
                        csv,
                ...
                {toolbar?.includes("print") && (
                  <Button
                    ...
                    buttonType="outlined"
                    severity="secondary"
                    ariaLabel={locale.dataTable?.print || "Print"}
                    tooltip={locale.dataTable?.print || "Print"}
                    onClick={() => {
                      const table = wrapperRef.current?.querySelector(
                        ".k-datatable__table"
                      );
      ...
      {(() => {
        ...
        const bodyEl = (
          <DataTableBody
            ...
            virtualScrollItemHeight={virtualScrollItemHeight}
        ...
        const footerEl = columns.some(c => c.footer || c.footerTemplate) ? (
          <tfoot>
            <tr role="row">
              {selectionMode === "checkbox" && (
                <td className="k-datatable__td" />
              )}
              {(!!rowExpansionTemplate || !!childrenField) && (
                <td className="k-datatable__td" />
              )}
              ...
              {columns
                ...
                .map((col, i) => (
                  <td
                    key={col.field + i}
                    className="k-datatable__td"
                    role="gridcell"
                  >
                    {col.footerTemplate
                      ? col.footerTemplate(col)
                      : col.footer || ""}
                  </td>
                ))}
            </tr>
        ...
        const tableEl = (
          <table className="k-datatable__table">
            {headerEl}
            {bodyEl}
            {footerEl}
          </table>
        ...
        if (scrollable && !virtualScroll) {
          return (
            <ScrollBar
              orientation="both"
              maxHeight={scrollHeight || "var(--kreati-dt-scroll-height)"}
              variant="kreati"
              size="sm"
              {...(scrollBarPropsProp as object)}
            >
              {tableEl}
            </ScrollBar>
          );
        ...
        if (virtualScroll) {
          return (
            <div
              className="k-datatable__virtual-scroll"
              style={{
                maxHeight: scrollHeight || "var(--kreati-dt-scroll-height)",
                overflow: "auto",
              }}
      ...
      {paginator && (
        <div className="k-datatable__paginator-bar">
          {paginatorTemplate ? (
            paginatorTemplate({
              page: currentPage,
              totalPages: Math.ceil(totalRecords / internalRows),
              totalRecords,
              rows: internalRows,
      ...
      {footerTemplate && (
        <div className="k-datatable__footer">{footerTemplate}</div>
      )}
    </div>
  );
