...
import { SORT_ASC_PATH, SORT_DESC_PATH, SORT_BOTH_PATH, FILTER_PATH } from '../iconPaths';
...
import type { DataTableColumn, SortDirection, SortMeta, ColumnFilterMeta, FilterMatchMode } from './types';

const SortIcon = ({ direction }: { direction: SortDirection }) => {
  const path = direction === 'asc' ? SORT_ASC_PATH : direction === 'desc' ? SORT_DESC_PATH : SORT_BOTH_PATH;
  return (
    <svg className="k-datatable__sort-icon" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
      <path d={path} />
    </svg>
  );
};
...
const FilterPanel: React.FC<FilterPanelProps> = ({ col, value, matchMode, onApply, onClear }) => {
  ...
  const { coords, positioned } = useOverlayPosition(
    ...
    open,
    { position: col.filterDirection === 'up' ? 'top' : 'bottom', offset: 8, matchTriggerWidth: false },
  );
  ...
  useEffect(() => {
    ...
    const handleClickOutside = (e: MouseEvent) => {
      ...
      if (target.closest('[class*="dropdown-portal"]') || target.closest('[class*="__dropdown"]')) return;
  ...
  const panel = open ? createPortal(
    <LayerContext.Provider value={nextLayer()}>
      ...
      >
        <div className="k-datatable__filter-panel__label">Filter: {col.header || col.field}</div>
        {col.filterTemplate ? (
          col.filterTemplate(col, localValue, (val) => { setLocalValue(val as string); onApply(val, localMode); setOpen(false); })
        ) : (
          <>
            <Select
            ...
            <Input
              ...
              onKeyDown={e => { if (e.key === 'Enter') { onApply(localValue, localMode); setOpen(false); } }}
              placeholder="Value..."
              size="xs"
              fullWidth
            />
        ...
        <div className="k-datatable__filter-panel__actions">
          <Button label="Clear" size="xs" buttonType="text" severity="secondary" onClick={() => { onClear(); setLocalValue(''); setOpen(false); }} />
          {!col.filterTemplate && <Button label="Apply" size="xs" severity="primary" onClick={() => { onApply(localValue, localMode); setOpen(false); }} />}
        </div>
      </div>
  ...
  return (
    <>
      <button
        ...
        aria-expanded={open}
      >
        <svg width="12" height="12" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><path d={FILTER_PATH} /></svg>
      </button>
      {panel}
    </>
  );
...
export const DataTableHeaderCell: React.FC<HeaderCellProps> = ({
  ...
  const style: React.CSSProperties = {
    ...
    ...(col.frozen === 'left' && frozenOffset !== undefined ? { left: frozenOffset } : {}),
    ...(col.frozen === 'right' && frozenOffset !== undefined ? { right: frozenOffset } : {}),
    ...col.headerStyle,
  };

  const handleClick = useCallback((e: React.MouseEvent) => {
    if (resizing) { e.stopPropagation(); return; }
    onSort?.();
  }, [resizing, onSort]);
  ...
  return (
    <th
      ...
      onKeyDown={col.sortable ? (e: React.KeyboardEvent) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); onSort?.(); } } : undefined}
    ...
    >
      <div className="k-datatable__th-content">
        {col.reorderable !== false && !!onDragStart && (
          <svg className="k-datatable__th-reorder-icon" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
            <path d="M8,6h2v12H8Zm6,0h2v12H14Z" />
          </svg>
        )}
        {col.headerTemplate ? col.headerTemplate(col) : <span>{col.header || col.field}</span>}
      ...
      {resizable && col.resizable !== false && (
        <div className={`k-datatable__resize-handle${resizing ? ' k-datatable__resize-handle--active' : ''}`} onMouseDown={handleResizeStart} />
      )}
    </th>
  );
};
...
interface HeaderProps {
  ...
  onFilterApply: (field: string, value: unknown, matchMode: FilterMatchMode) => void;
...
export const DataTableHeader: React.FC<HeaderProps> = ({
  ...
  const getSortDirection = (field: string): SortDirection => {
    if (sortMode === 'single') return sortField === field ? sortOrder ?? null : null;
  ...
  const groupRow = columnGroups && columnGroups.size > 0 ? (() => {
    const cells: React.ReactNode[] = [];
    if (selectionMode === 'checkbox') cells.push(<th key="__sel-g" className="k-datatable__th" />);
    if (hasExpansion) cells.push(<th key="__exp-g" className="k-datatable__th" />);
    if (reorderableRows) cells.push(<th key="__drag-g" className="k-datatable__th" />);
    let currentGroup = '';
    let span = 0;
    ...
    visibleCols.forEach((col, i) => {
      ...
      if (group === currentGroup) {
        ...
        if (span > 0) cells.push(<th key={`g-${i - span}`} className="k-datatable__th" colSpan={span}>{currentGroup}</th>);
        currentGroup = group;
        span = 1;
      }
      if (i === visibleCols.length - 1) {
        cells.push(<th key={`g-${i - span + 1}`} className="k-datatable__th" colSpan={span}>{currentGroup}</th>);
      }
    });
  ...
  return (
    <thead className="k-datatable__thead" role="rowgroup">
      ...
      <tr role="row">
        ...
        {reorderableRows && <th className="k-datatable__th k-datatable__drag-cell" aria-label="Reorder" />}
        {hasExpansion && <th className="k-datatable__th k-datatable__expand-cell" aria-label="Expand" />}
        {visibleCols.map((col, i) => (
          <DataTableHeaderCell
            ...
            filterMatchMode={filters[col.field]?.matchMode || col.filterMatchMode || 'contains'}
            onFilterApply={col.filterable ? (val, mode) => onFilterApply(col.field, val, mode) : undefined}
            onFilterClear={col.filterable ? () => onFilterClear(col.field) : undefined}
            onDragStart={reorderableColumns ? handleDragStart(i + selOffset + dragOffset) : undefined}
            ...
            resizable={tableResizable}
            onResize={onResize}
            frozenOffset={col.frozen === 'left' ? frozenLeftOffsets[col.field] : col.frozen === 'right' ? frozenRightOffsets[col.field] : undefined}
          />
        ))}
      </tr>
    </thead>
