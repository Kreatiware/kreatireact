...
export interface SegmentedControlProps {
  ...
  pillTemplate?: (activeIndex: number, pillStyle: React.CSSProperties) => React.ReactNode;
  /** Custom render for each option. Receives option, index, and active state. */
  optionTemplate?: (option: SegmentedControlOption, index: number, active: boolean) => React.ReactNode;
...
export const SegmentedControl = forwardRef<HTMLDivElement, SegmentedControlProps>(
  (
    ...
    const updatePill = useCallback(() => {
      ...
      const btn = container.querySelectorAll<HTMLButtonElement>('.k-segmented__option')[activeIndex];
      if (!btn) return;
      setPillStyle({
        ...
        <div className="k-segmented__track" ref={containerRef}>
          {activeIndex >= 0 && (
            pillTemplate
              ...
              : <div className="k-segmented__pill" style={pillStyle} aria-hidden="true" />
          )}
          {options.map((opt, i) => {
                ...
                onKeyDown={(e) => {
                  ...
                  if (e.key === 'ArrowRight' || e.key === 'ArrowDown') {
                    ...
                    for (let j = 1; j < len; j++) {
                      ...
                      if (!options[idx].disabled) { next = idx; break; }
                    }
                  ...
                  if (next >= 0) {
                    ...
                    const btns = containerRef.current?.querySelectorAll<HTMLButtonElement>('.k-segmented__option');
                    btns?.[next]?.focus();
                  }
                }}
              >
                {optionTemplate
                  ...
                  : <>
                      {opt.icon && <span className="k-segmented__icon" aria-hidden="true">{opt.icon}</span>}
                      {opt.label && <span className="k-segmented__label">{opt.label}</span>}
                    </>
                }
              </button>
            );
