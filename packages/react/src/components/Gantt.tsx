 ...
 * ```
 */
const Gantt = forwardRef<GanttRef, GanttProps>(
  (
    {
      tasks: tasksProp,
      defaultTasks,
      ...
      if (!isControlled) {
        setInternalTasks(prev => {
          ...
          if (autoScheduleRef.current)
            next = cascadeRef.current(next, draft.id);
          return next;
        });
      }
      setEditingTask(null);
    ...
    const criticalPathIds = useMemo(() => {
      ...
      for (const tk of nonSummary) {
        ...
        ls.set(
          tk.id,
          projectEnd - ((ef.get(tk.id) ?? 0) - (es.get(tk.id) ?? 0))
        );
      }
      changed = true;
      iter = 0;
    ...
    const [autoScheduleEnabled, setAutoScheduleEnabled] =
      useState(autoSchedule);
    ...
    const xToDate = useCallback(
      (x: number): Date => {
        ...
        return addDays(
          dateRange.start,
          Math.round((x / totalWidth) * totalDays)
        );
      },
      [dateRange, viewMode, cw, totalWidth]
    );
    ...
    const handleSplitterDown = useCallback(
      (e: React.MouseEvent) => {
        ...
        const onMove = (ev: MouseEvent) => {
          setTblWidth(
            Math.max(minTableWidth, startWidth + ev.clientX - startX)
          );
        };
        const onUp = () => {
              ...
              if (!isControlled) {
                setInternalTasks(prev => {
                  ...
                  if (autoScheduleEnabled)
                    next = cascadeSchedule(next, task.id);
                  return next;
                });
              }
              onTaskChange?.({
    ...
    const handleConnectorDown = useCallback(
      (e: React.MouseEvent, taskId: string, side: "start" | "finish") => {
        ...
        const onMove = (ev: MouseEvent) => {
          ...
          if (!c) return;
          const snap = findNearestConnector(
            c.x,
            c.y,
            linkDragRef.current.fromId
          );
          setLinkDragLine({
        ...
        const onUp = (ev: MouseEvent) => {
          ...
          const depType = `${fromLetter}${toLetter}` as
            | "FS"
            | "FF"
            | "SS"
            | "SF";
      ...
      (e: React.DragEvent) => {
        ...
        if (!isControlled) {
          setInternalTasks(prev => {
            ...
            const insertIdx =
              rowDropPosition === "above" ? dropIdx : dropIdx + 1;
          ...
          if (task.type === "milestone") {
            ...
            return (
              <g
                ...
                onContextMenu={e =>
                  editable && handleRowContext(e, task, false)
                }
                onMouseDown={e => {
                ...
                {editable && (
                  <>
                    <circle
                      ...
                      r={5}
                      onMouseDown={e =>
                        handleConnectorDown(e, task.id, "start")
                      }
                    />
                    <circle
                      className="k-gantt__link-connector"
                      ...
                      r={5}
                      onMouseDown={e =>
                        handleConnectorDown(e, task.id, "finish")
                      }
                    />
                  </>
                )}
                      ...
                      .filter(
                        tk =>
                          ...
                          !(editDraft.dependencies ?? []).some(
                            d =>
                              (typeof d === "string" ? d : d.taskId) === tk.id
                          )
                      )
                  ...
                  const displayed = successors
                    ...
                    .map(tk => {
                      const pending = pendingSuccessorChanges.find(
                        ch =>
                          ch.targetTaskId === tk.id && ch.action === "update"
                      );
