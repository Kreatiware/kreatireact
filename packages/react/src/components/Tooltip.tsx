...
export const Tooltip = forwardRef<HTMLDivElement, TooltipProps>(
  (
    ...
    const tooltipId = useRef(
      `k-tooltip-${Math.random().toString(36).slice(2, 9)}`
    ).current;
    ...
    const computePosition = useCallback(() => {
      ...
      if (mouseTrack) {
        setCoords({
          top: mouseRef.current.y + mouseTrackTop,
          left: mouseRef.current.x + mouseTrackLeft,
        });
      ...
      else if (pos === "bottom" && tr.bottom + gap + br.height > vh)
        pos = "top";
