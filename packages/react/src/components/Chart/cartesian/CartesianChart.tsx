...
export const CartesianChart = ({
  ...
  children,
  ref,
}: CartesianChartProps & { ref?: React.Ref<HTMLDivElement> }) => {
    ...
    (e: React.MouseEvent) => {
      ...
      if (isSelecting) {
        ...
        const moveHandler = (ev: MouseEvent) => {
          ...
          const cx =
            zoomAxis === "y" ? plotWidth : Math.max(0, Math.min(mx, plotWidth));
          const cy =
            zoomAxis === "x"
              ? plotHeight
  ...
  const handleSvgMouseMove = useCallback(
    (e: React.MouseEvent) => {
      if (isPanning && panStart) {
        ...
        setSelectCurrent({
          x:
            zoomAxis === "y" ? plotWidth : Math.max(0, Math.min(px, plotWidth)),
          y:
            zoomAxis === "x"
              ? plotHeight
                      ...
                      .filter((a, ai) => {
                        ...
                        const oHave = yAxes.some(
                          (o, oi) =>
                            ...
                            visibleSeries.some(
                              s =>
                                (s.yAxisId ?? "default") === (o.id ?? "default")
                            )
                        );
                        return aHas || !oHave;
                      }).length;
