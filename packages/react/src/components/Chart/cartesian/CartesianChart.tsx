...
export interface CartesianChartProps {
  ...
  legendPosition?: "top" | "bottom" | "left" | "right" | "inside-top-right" | "inside-top-left";
  ...
  onSelectChange?: (range: { x1: number; x2: number; plotLeft: number; plotWidth: number } | null) => void;
  ...
  contextMenuRender?: (defaultItems: MenuItem[], onSelect: (key: string) => void, children: React.ReactNode) => React.ReactNode;
...
export const CartesianChart = forwardRef<HTMLDivElement, CartesianChartProps>((
  ...
  const [mousePos, setMousePos] = useState<{ plotX: number; plotY: number } | null>(null);
  ...
  const zoomState = controlledZoom !== undefined ? controlledZoom : internalZoom;
  const setZoomState = useCallback((v: ZoomState | null | ((prev: ZoomState | null) => ZoomState | null)) => {
  ...
  const yAxes: ChartAxisConfig[] = useMemo(() => {
    ...
    return Array.isArray(yAxisConfig) ? (yAxisConfig.length ? yAxisConfig : [{}]) : [yAxisConfig];
  }, [yAxisConfig]);

  const yConfig = yAxes[0];

  // Compute domains — primary Y from all series, per-axis from matched series
  const { xDomain: fullXDomain, yDomain: fullYDomain, yDomains } = useMemo(() => {
    ...
    for (const s of visibleSeries) {
      ...
      if (!axisRanges[axId]) axisRanges[axId] = { min: Infinity, max: -Infinity };
    ...
    for (const ax of yAxes) {
      ...
      if (!isFinite(r.min)) { r.min = 0; r.max = 1; }
      domains[id] = niceDomain(ax.min ?? r.min, ax.max ?? r.max);
    }
    const primaryId = yAxes[0].id ?? "default";
    return {
      xDomain: [xAxisConfig?.min ?? xMin, xAxisConfig?.max ?? xMax] as [number, number],
  ...
  const fullDomain: ZoomState = useMemo(() => ({
    xMin: fullXDomain[0], xMax: fullXDomain[1],
    yMin: fullYDomain[0], yMax: fullYDomain[1],
  ...
  const yDomain: [number, number] = controlledZoom !== undefined && zoomAxis === "x"
    ? fullYDomain
    : [activeZoom.yMin, activeZoom.yMax];
  const zoomed = zoomState != null && (controlledZoom !== undefined && zoomAxis === "x"
    ? (Math.abs(zoomState.xMin - fullDomain.xMin) > 0.001 || Math.abs(zoomState.xMax - fullDomain.xMax) > 0.001)
  ...
  const visibleYAxes = yAxes.filter((ax, i) => {
    ...
    const othersHave = yAxes.some((a, j) => j !== i && visibleSeries.some((s) => (s.yAxisId ?? "default") === (a.id ?? "default")));
    return has || !othersHave;
  });
  const rightAxesCount = visibleYAxes.filter((a) => a.side === "right").length;
  const leftAxesCount = Math.max(1, visibleYAxes.filter((a) => a.side !== "right").length);
  const effectiveMargins: ChartMargins = {
    top: margins?.top ?? 20,
    right: margins?.right ?? (20 + (rightAxesCount > 0 ? 60 * rightAxesCount : 0)),
  ...
  const svgWidth = resizeWidth ?? (typeof width === "number" ? width : measuredWidth);
  const clampedWidth = Math.max(minWidth ?? 0, Math.min(svgWidth, maxWidth ?? Infinity));
  const resolvedHeight = resizeHeight ?? height;
  const clampedHeight = Math.max(minHeight ?? 0, Math.min(resolvedHeight, maxHeight ?? Infinity));
  const plotWidth = Math.max(clampedWidth - effectiveMargins.left - effectiveMargins.right, 0);
  const plotHeight = Math.max(clampedHeight - effectiveMargins.top - effectiveMargins.bottom, 0);

  const xScale = useMemo(
    () => createScale(xAxisConfig?.type || "linear", xDomain, [0, Math.max(plotWidth, 0)], xAxisConfig?.categories),
  ...
  const yScales = useMemo(() => {
    ...
    for (const ax of yAxes) {
      ...
      if (zoomState && id !== (yAxes[0].id ?? "default")) {
        ...
        const zoomFracMin = (activeZoom.yMin - primaryDomain[0]) / (primaryDomain[1] - primaryDomain[0]);
        const zoomFracMax = (activeZoom.yMax - primaryDomain[0]) / (primaryDomain[1] - primaryDomain[0]);
  ...
  const handleKeyDown = useCallback(
    ...
    [visibleSeries, focusedSeriesIndex, focusedPointIndex, onPointClick, announcePoint]
  );
  ...
  const handleWheel = useCallback(
    (e: React.WheelEvent) => {
      ...
      const fracX = (e.clientX - rect.left - effectiveMargins.left) / plotWidth;
      const fracY = (e.clientY - rect.top - effectiveMargins.top) / plotHeight;
      setZoomState((prev) => wheelZoom(prev ?? fullDomain, fullDomain, fracX, fracY, e.deltaY, zoomAxis));
    },
  ...
  const handleSvgMouseDown = useCallback(
    (e: React.MouseEvent) => {
      ...
      if (e.ctrlKey && canSelect) {
        setSelectStart({ x: zoomAxis === "y" ? 0 : px, y: zoomAxis === "x" ? 0 : py });
        setSelectCurrent({ x: zoomAxis === "y" ? plotWidth : px, y: zoomAxis === "x" ? plotHeight : py });
        isSelecting = true;
      } else if (zoomed) {
        setIsPanning(true);
      ...
      if (isSelecting) {
        ...
        const moveHandler = (ev: MouseEvent) => {
          ...
          const cx = zoomAxis === "y" ? plotWidth : Math.max(0, Math.min(mx, plotWidth));
          const cy = zoomAxis === "x" ? plotHeight : Math.max(0, Math.min(my, plotHeight));
        ...
        if (globalMouseMoveRef.current) document.removeEventListener("mousemove", globalMouseMoveRef.current);
        globalMouseMoveRef.current = moveHandler;
        document.addEventListener("mousemove", moveHandler);
      }

      if (globalMouseUpRef.current) document.removeEventListener("mouseup", globalMouseUpRef.current);
      const upHandler = () => {
        svgMouseUpRef.current();
        if (globalMouseMoveRef.current) { document.removeEventListener("mousemove", globalMouseMoveRef.current); globalMouseMoveRef.current = null; }
  ...
  const handleSvgMouseMove = useCallback(
    (e: React.MouseEvent) => {
      if (isPanning && panStart) {
        ...
        const py = e.clientY - rect.top - effectiveMargins.top;
        setSelectCurrent({
          x: zoomAxis === "y" ? plotWidth : Math.max(0, Math.min(px, plotWidth)),
          y: zoomAxis === "x" ? plotHeight : Math.max(0, Math.min(py, plotHeight)),
        });
      }
    },
    [isPanning, panStart, selectStart, plotWidth, plotHeight, fullDomain, effectiveMargins, zoomAxis]
  );
  ...
  const handleSvgMouseUp = useCallback(() => {
    if (isPanning) {
      ...
      const hasDrag = zoomAxis === "x" ? dx > 5 : zoomAxis === "y" ? dy > 5 : (dx > 5 || dy > 5);
      if (hasDrag) {
        ...
        if (zoomAxis === "x") { y1 = 0; y2 = 1; }
        if (zoomAxis === "y") { x1 = 0; x2 = 1; }
  ...
  }, [isPanning, selectStart, selectCurrent, plotWidth, plotHeight, fullDomain, zoomState, zoomAxis, onSelectChange]);
  ...
  const touchRef = useRef<{ startTouches: { clientX: number; clientY: number }[]; startZoom: ZoomState } | null>(null);

  const handleTouchStart = useCallback(
    (e: React.TouchEvent) => {
      if (!zoomMode || e.touches.length < 1) return;
      touchRef.current = {
        startTouches: Array.from(e.touches).map((t) => ({ clientX: t.clientX, clientY: t.clientY })),
  ...
  const handleTouchMove = useCallback(
    (e: React.TouchEvent) => {
      ...
      if (e.touches.length === 1 && startTouches.length === 1 && zoomed) {
        // Single finger pan
        e.preventDefault();
        const dx = (e.touches[0].clientX - startTouches[0].clientX) / plotWidth;
        const dy = (e.touches[0].clientY - startTouches[0].clientY) / plotHeight;
        ...
        const svg = (e.target as Element).closest?.("svg");
        const rect = svg?.getBoundingClientRect();
        const fracX = rect ? (midX - rect.left - effectiveMargins.left) / plotWidth : 0.5;
        ...
        setZoomState({ xMin, xMax, yMin: startZoom.yMin, yMax: startZoom.yMax });
      }
    },
  ...
  const isNavVisible = navigatorVisibility === "fixed" ||
  ...
  const legend = showLegend && series.length > 1 ? (
    <Legend
      ...
      position={isInsideLegend ? "top" : (legendPosition as "top" | "bottom" | "left" | "right")}
  ...
  // Resize drag handler
  const handleResizeStart = useCallback((axis: "vertical" | "horizontal" | "both", e: React.MouseEvent) => {
    e.preventDefault();
    ...
    const onMove = (ev: MouseEvent) => {
      if (axis !== "horizontal") {
        ...
        setResizeHeight(Math.max(minHeight ?? 100, Math.min(newH, maxHeight ?? Infinity)));
      }
      if (axis !== "vertical") {
        const newW = startW + (ev.clientX - startX);
        setResizeWidth(Math.max(minWidth ?? 200, Math.min(newW, maxWidth ?? Infinity)));
      }
    };
    const onUp = () => {
    ...
    >
      ...
      {(title || subtitle) && (
        <div className="k-chart-header">
          ...
          {subtitle && <div className="k-chart-header__subtitle">{subtitle}</div>}
        </div>
      )}
      {legendPosition === "top" && legend}
      <div className="k-cartesian-chart__body" style={legendPosition === "left" || legendPosition === "right" ? { display: "flex" } : undefined}>
        {legendPosition === "left" && legend}
        {(() => {
          const defaultItems: MenuItem[] = [
            ...(zoomed ? [
              { key: "reset-zoom", label: t.resetZoom },
              ...((exportFormats.length > 0 || contextMenuItems?.length) ? [{ key: "divider-1", separator: true }] : []),
            ] : []),
            ...(exportFormats.includes("png") ? [{ key: "export-png", label: t.exportPng }] : []),
            ...(exportFormats.includes("svg") ? [{ key: "export-svg", label: t.exportSvg }] : []),
            ...(exportFormats.includes("csv") ? [{ key: "export-csv", label: t.exportCsv }] : []),
            ...(exportFormats.includes("json-table") ? [{ key: "export-json-table", label: t.exportJsonTable }] : []),
            ...(exportFormats.includes("json-series") ? [{ key: "export-json-series", label: t.exportJsonSeries }] : []),
            ...(contextMenuItems?.length ? [
              ...((exportFormats.length > 0) ? [{ key: "divider-custom", separator: true }] : []),
              ...contextMenuItems,
            ] : []),
          ];
          const handleMenuSelect = (key: string) => {
            if (key === "reset-zoom") resetZoom();
            const svg = containerRef.current?.querySelector("svg.k-chart") as SVGSVGElement | null;
            if (key === "export-png" && svg) exportPng(svg, exportTitle ? title : undefined, exportTitle ? subtitle : undefined);
            if (key === "export-svg" && svg) exportSvg(svg, exportTitle ? title : undefined, exportTitle ? subtitle : undefined);
            if (key === "export-csv") exportCsv(visibleSeries, xAxisConfig?.categories, xAxisConfig?.label, csvSeparator);
            if (key === "export-json-table") exportJsonTable(visibleSeries, xAxisConfig?.categories, xAxisConfig?.label);
        ...
        <div
          ...
          style={{ flex: 1, position: "relative", cursor: isPanning ? "grabbing" : zoomed && ctrlHeld ? "crosshair" : zoomed ? "grab" : zoomMode ? "crosshair" : onPointClick ? "pointer" : undefined }}
          onWheel={handleWheel}
          onMouseDown={handleSvgMouseDown}
          onMouseUp={handleSvgMouseUp}
          onMouseLeave={() => { if (!selectStart) handleSvgMouseUp(); setMousePos(null); if (onCrosshairChange) onCrosshairChange(null); }}
          onMouseMove={(e) => { handleSvgMouseMove(e); handleChartMouseMove(e); }}
          onTouchStart={handleTouchStart}
          onTouchMove={handleTouchMove}
          onTouchEnd={handleTouchEnd}
        >
          {/* Chart menu button */}
          {(showMenuButton === true || (showMenuButton === "auto" && (zoomed || exportFormats.length > 0 || (contextMenuItems && contextMenuItems.length > 0)))) && (
            <button
              type="button"
          ...
          <ChartCanvas
            ...
            onMouseMove={(e) => {
              ...
              if (onCrosshairChange) {
                ...
                const rawX = d0 + (e.plotX - r0) / (r1 - r0) * (d1 - d0);
                ...
                for (const s of visibleSeries) {
                  for (const p of s.data) {
                    ...
                    if (dist < minDist) { minDist = dist; nearest = p.x; }
                  }
                }
                onCrosshairChange(nearest);
              }
            }}
            onMouseLeave={() => { setMousePos(null); if (onCrosshairChange) onCrosshairChange(null); }}
          >
            {/* Y Axes */}
            {(() => {
              ...
              yAxes.forEach((ax, axIdx) => {
                const id = ax.id ?? "default";
                const hasVis = visibleSeries.some((s) => (s.yAxisId ?? "default") === id);
                const othersHave = yAxes.some((a, i) => i !== axIdx && visibleSeries.some((s) => (s.yAxisId ?? "default") === (a.id ?? "default")));
                ...
                const shouldShowGrid = side === "left" && !gridAssigned && (ax.showGrid ?? showGrid);
                if (shouldShowGrid) gridAssigned = true;
                const axCfg: ChartAxisConfig = { ...ax, showGrid: shouldShowGrid };

                const visibleAxesBefore = yAxes.slice(0, axIdx).filter((a, ai) => {
                  const aId = a.id ?? "default";
                  const aSide = a.side ?? (ai === 0 ? "left" : "right");
                  if (aSide !== side) return false;
                  const aHas = visibleSeries.some((s) => (s.yAxisId ?? "default") === aId);
                  const oHave = yAxes.some((o, oi) => oi !== ai && visibleSeries.some((s) => (s.yAxisId ?? "default") === (o.id ?? "default")));
                  return aHas || !oHave;
                }).length;
                const offset = visibleAxesBefore * 60;

                rendered.push(
                  <g key={id} transform={side === "right" ? `translate(${plotWidth + offset},0)` : `translate(${-offset},0)`}>
                    <Axis orientation={orientation} scale={scale} config={axCfg} plotWidth={Math.max(plotWidth, 0)} plotHeight={Math.max(plotHeight, 0)} />
                  </g>
                );
              });
              ...
              if (!hasLeftAxis && rendered.length > 0) {
                const firstVisibleScale = yScales[yAxes.find((a) => visibleSeries.some((s) => (s.yAxisId ?? "default") === (a.id ?? "default")))?.id ?? "default"] ?? yScale;
                rendered.unshift(
                  <g key="__ghost-left">
                    <Axis
                      orientation="left"
            ...
            {annotations && annotations.length > 0 && (
              <defs>
                ...
                >
                  <path d="M 0 0 L 10 5 L 0 10 Z" fill="var(--kreati-chart-text)" />
                </marker>
              </defs>
            )}
          ...
          {isInsideLegend && legend && (
            <div
              ...
              style={{ "--k-chart-margin-left": `${effectiveMargins.left}px` } as React.CSSProperties}
            >
              {legend}
            </div>
          )}
          {/* Range navigator — inside position */}
          {showNavigator && navigatorPosition === "inside" && svgWidth > 0 && (
            <div
              className={`k-chart-navigator-wrap k-chart-navigator-wrap--inside ${isNavVisible ? "k-chart-navigator-wrap--visible" : ""}`}
              onMouseEnter={() => { setNavVisible(true); if (navTimerRef.current) clearTimeout(navTimerRef.current); }}
              onMouseLeave={() => { if (navigatorVisibility === "auto") navTimerRef.current = setTimeout(() => setNavVisible(false), 3500); }}
            >
              <ChartNavigator
                series={series}
                xDomain={fullXDomain}
      ...
      <table className="k-sr-only" role="table" aria-label={`${title ?? "Chart"} data`}>
        <thead>
          <tr>
            <th scope="col">{xAxisConfig?.label ?? "X"}</th>
            {visibleSeries.map((s) => (
              <th key={s.id} scope="col">{s.name}{s.unit ? ` (${s.unit})` : ""}</th>
            ))}
          </tr>
        </thead>
        <tbody>
          {(() => {
            const maxLen = Math.max(...visibleSeries.map((s) => s.data.length), 0);
            const rows: React.ReactNode[] = [];
            for (let i = 0; i < maxLen; i++) {
              const xVal = visibleSeries[0]?.data[i]?.x;
              const xLabel = xAxisConfig?.categories?.[xVal ?? i] ?? String(xVal ?? i);
              rows.push(
                <tr key={i}>
                  <td>{xLabel}</td>
                  {visibleSeries.map((s) => (
