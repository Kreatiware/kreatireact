...
const ICON_RECT_MAP: Record<string, readonly { x: number; y: number; width: number; height: number }[]> = {
...
export const resolveIcon = (name: string, size = 16): React.ReactNode => {
  ...
  if (rects) {
    return (
      <svg width={size} height={size} viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
        {rects.map((r, i) => (
          <rect key={i} x={r.x} y={r.y} width={r.width} height={r.height} rx=".57" ry=".57" />
        ))}
      </svg>
    );
  }
 ...
 */
export const renderMenuIcon = (icon: React.ReactNode, size = 16): React.ReactNode => {
