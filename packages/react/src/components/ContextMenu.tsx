...
const SubmenuPanel: React.FC<{
  ...
  useEffect(() => {
    ...
    if (position === 'right' && rect.right > window.innerWidth) resolved = 'left';
    else if (position === 'left' && rect.left < 0) resolved = 'right';
    else if (position === 'bottom' && rect.bottom > window.innerHeight) resolved = 'top';
    else if (position === 'top' && rect.top < 0) resolved = 'bottom';
    // Secondary axis overflow
    if ((resolved === 'right' || resolved === 'left') && rect.bottom > window.innerHeight) {
      setPos(resolved);
      el.style.top = 'auto';
      el.style.bottom = '0';
      return;
...
export const ContextMenu = forwardRef<HTMLDivElement, ContextMenuProps>(
  (
    ...
    const calcPosition = useCallback((rect: DOMRect) => {
      switch (placement) {
        case 'right': return { x: rect.right + 2, y: rect.top };
        case 'left': return { x: rect.left, y: rect.top };
        case 'top': return { x: rect.left, y: rect.top };
        default: return { x: rect.left, y: rect.bottom + 2 };
      }
    }, [placement]);
    ...
    useEffect(() => {
      ...
      let { x, y } = position;
      let changed = false;
      if (x + rect.width > window.innerWidth) { x = window.innerWidth - rect.width - 8; changed = true; }
      if (y + rect.height > window.innerHeight) { y = window.innerHeight - rect.height - 8; changed = true; }
    ...
    const handleItemKeyDown = useCallback(
      (item: MenuItem, e: React.KeyboardEvent, siblings: MenuItem[]) => {
        const enabled = siblings.filter((i) => !i.separator && !i.disabled && i.visible !== false);
        const idx = enabled.findIndex((i) => i.key === item.key);

        if (e.key === 'ArrowDown') {
          e.preventDefault();
          const next = enabled[(idx + 1) % enabled.length];
          (document.getElementById(`${base}-item-${next.key}`) as HTMLElement)?.focus();
        } else if (e.key === 'ArrowUp') {
          e.preventDefault();
          const prev = enabled[(idx - 1 + enabled.length) % enabled.length];
          (document.getElementById(`${base}-item-${prev.key}`) as HTMLElement)?.focus();
        } else if (e.key === 'ArrowRight' && item.items && item.items.length > 0) {
          e.preventDefault();
          setOpenSubmenus((prev) => new Set(prev).add(item.key));
          setTimeout(() => {
            const first = item.items!.find((i) => !i.separator && !i.disabled && i.visible !== false);
            if (first) (document.getElementById(`${base}-item-${first.key}`) as HTMLElement)?.focus();
          }, 0);
      ...
      return visible.map((item) => {
          ...
          return (
            <React.Fragment key={item.key}>
              ...
              >
                ...
                {hasChildren && (
                  <span className={`${base}__arrow ${isSubOpen ? `${base}__arrow--open` : ''}`} aria-hidden="true">
                    <svg width={12} height={12} viewBox="0 0 24 24" fill="currentColor"><path d={CHEVRON_RIGHT_PATH} /></svg>
                  </span>
                )}
              </button>
        ...
        return (
          <div
            ...
            onMouseLeave={() => {
              if (hasChildren && !('ontouchstart' in window)) {
                setOpenSubmenus((prev) => { const n = new Set(prev); n.delete(item.key); return n; });
              }
            }}
          >
            <button
            ...
            {hasChildren && isSubOpen && (
              <SubmenuPanel base={base} position={item.submenuPosition || submenuPosition}>
        ...
        {open &&
          createPortal(
            <div
              ...
              style={isMobile ? { zIndex } : { left: position.x, top: position.y, zIndex }}
              role="menu"
              aria-label="Context menu"
              onClick={(e) => e.stopPropagation()}
              onContextMenu={(e) => e.stopPropagation()}
            >
              {panelTemplate ? panelTemplate(visibleItems, close) : renderItems(visibleItems)}
            </div>,
            document.body,
          )}
      </div>
