...
export const Kanban = ({
  ...
  style,
  ref,
}: KanbanProps & { ref?: React.Ref<HTMLDivElement> }) => {
  ...
  const [internalOrder, setInternalOrder] = useState<Record<string, string[]>>(
    () => defaultCardOrder ?? {}
  );
  ...
  const handleCreateSubmit = useCallback(() => {
    ...
    if (createSwimlaneId.current) newCard.swimlaneId = createSwimlaneId.current;
  ...
  const handleDeleteConfirm = useCallback(() => {
    ...
    closeDetail();
  }, [confirmDeleteCard, updateCards, updateOrder, onCardDelete, closeDetail]);
  ...
  const toggleColumnCollapse = useCallback(
    (colId: string) => {
      updateColumns(prev =>
        prev.map(c => (c.id === colId ? { ...c, collapsed: !c.collapsed } : c))
      );
    },
    [updateColumns]
  );
  ...
  const getColumnContextItems = useCallback(
    (colId: string) => {
      ...
      return [
        ...
        {
          key: "collapse",
          label: col?.collapsed ? locale.kanban.expand : locale.kanban.collapse,
  ...
  const renderForm = () => (
    <div className="k-kanban-dialog__form">
      <Input
        ...
        value={formTitle}
        onChange={e =>
          setFormTitle((e as React.ChangeEvent<HTMLInputElement>).target.value)
        }
        fullWidth
        size="sm"
        required
  ...
  const renderView = (card: KanbanCard) => (
    <div className="k-kanban-dialog__view">
      ...
      {card.description && (
        <p className="k-kanban-dialog__view-description">{card.description}</p>
      )}
  ...
  const renderColumn = (
    ...
    const isOverLimit = column.maxCards != null && cardCount > column.maxCards;
