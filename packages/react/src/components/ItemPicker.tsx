...
import {
  ...
  CHEVRON_UP_PATH,
  CHEVRON_DOWN_PATH,
  DOUBLE_ARROW_RIGHT_PATH,
  DOUBLE_ARROW_LEFT_PATH,
} from "./iconPaths";
...
export const ItemPicker = ({
  ...
  style,
  ref,
}: ItemPickerProps & { ref?: React.Ref<HTMLDivElement> }) => {
  ...
  const resolvedFilterPlaceholder =
    filterPlaceholder ?? locale.itemPicker.filterPlaceholder;
  ...
  useMemo(() => {
    ...
    const added = items
      .filter(i => !prevItemKeys.current.has(i.value))
      .map(i => i.value);
    if (added.length > 0) {
      setSourceOrder(prev => [
        ...prev.filter(v => currentKeys.has(v)),
        ...added,
      ]);
    } else {
  ...
  const sourceItems = useMemo(
    () =>
      sourceOrder
        .filter(v => !targetSet.has(v) && itemMap.has(v))
        .map(v => itemMap.get(v)!),
  ...
  const filteredSource = useMemo(
    () => filterList(sourceItems, sourceFilter),
    [sourceItems, sourceFilter, filterList]
  );
  const filteredTarget = useMemo(
    () => filterList(targetItems, targetFilter),
    [targetItems, targetFilter, filterList]
  );

  const toggleSelection = useCallback(
    (setFn: React.Dispatch<React.SetStateAction<Set<string>>>, val: string) => {
      setFn(prev => {
  ...
  const moveToTarget = useCallback(() => {
    const toMove = [...sourceSelected].filter(
      v => itemMap.has(v) && !targetSet.has(v)
    );
  ...
  const reorderDown = useCallback(
    (arr: string[], sel: Set<string>): string[] => {
      const next = [...arr];
  ...
  const moveTargetUp = useCallback(
    () => emit(reorderUp(value, targetSelected)),
    [value, targetSelected, reorderUp, emit]
  );
  const moveTargetDown = useCallback(
    () => emit(reorderDown(value, targetSelected)),
    [value, targetSelected, reorderDown, emit]
  );
  ...
  const handleDragStart = useCallback(
    (e: React.DragEvent, item: ItemPickerItem, from: "source" | "target") => {
      if (disabled) {
        e.preventDefault();
        return;
      }
  ...
  const getDropValue = useCallback(
    (e: React.DragEvent, panelEl: HTMLElement): string | null => {
      const listEl = panelEl.querySelector('[role="listbox"]');
      if (!listEl) return null;
      const options = Array.from(
        listEl.querySelectorAll<HTMLElement>('[role="option"]')
      );
      for (const opt of options) {
        const rect = opt.getBoundingClientRect();
        if (e.clientY < rect.top + rect.height / 2)
          return opt.dataset.value ?? null;
      }
      return null;
    },
    []
  ...
  const getVisualIndex = useCallback(
    (e: React.DragEvent, panelEl: HTMLElement): number => {
      const listEl = panelEl.querySelector('[role="listbox"]');
      if (!listEl) return 0;
      const options = Array.from(
        listEl.querySelectorAll<HTMLElement>('[role="option"]')
  ...
  const handleDragOver = useCallback(
    (e: React.DragEvent, panel: "source" | "target") => {
      ...
      if (dd.from === panel) {
        const canReorder =
          (panel === "source" &&
            (reorderable === "source" || reorderable === "both")) ||
          (panel === "target" &&
            (reorderable === "target" || reorderable === "both"));
        ...
        if (panel === "target") {
          ...
          setSourceOrder(prev => {
            ...
            const result = insertAt(
              sourceVals,
              sourceVals.filter(v => dragSet.has(v))
            );
            const out: string[] = [];
            let si = 0;
            for (const v of prev) {
              if (targetSet.has(v)) out.push(v);
              else out.push(result[si++]);
            }
            return out;
          });
        }
    ...
    (
      ...
      switch (e.key) {
        ...
        case "Enter": {
          ...
          if (v) toggleSelection(setSelected, v);
          return;
        }
        default:
          return;
  ...
  const renderItem = (
    ...
    const showDropLine =
      dropIndicator?.panel === from && dropIndicator.index === indexInList;
    const cls = [
      `${base}__item`,
    ...
    return (
      <div
        ...
        onClick={
          isDis ? undefined : () => toggleSelection(setSelected, item.value)
        }
        onKeyDown={
          isDis
            ? undefined
      ...
      >
        ...
        {itemTemplate ? (
          itemTemplate(item, { selected: isSel, disabled: isDis })
        ) : (
          <>
            {item.icon && (
              <span className={`${base}__item-icon`} aria-hidden="true">
                {item.icon}
              </span>
            )}
  ...
  const renderPanel = (
    ...
    const panelCls = [
      `${base}__panel`,
      isDropTarget && `${base}__panel--drop-active`,
    ]
      .filter(Boolean)
      .join(" ");
    ...
    const showDropEnd =
      isDropTarget && dropIndicator!.index === filteredItems.length;
      ...
      >
          ...
          <span className={`${base}__header-actions`}>
            {onUp && onDown && (
              <>
                <button
                  type="button"
                  className={`${base}__reorder-btn`}
                  onClick={onUp}
                  disabled={disabled || !hasSel}
                  aria-label={locale.itemPicker.moveUp}
                  title={locale.itemPicker.moveUp}
                >
                ...
                <button
                  ...
                  onClick={onDown}
                  disabled={disabled || !hasSel}
                  aria-label={locale.itemPicker.moveDown}
                  title={locale.itemPicker.moveDown}
                >
            ...
            {showCount && (
              <span className={`${base}__header-count`}>
                {locale.itemPicker.itemCount.replace(
                  "{count}",
        ...
        >
          {filteredItems.length === 0 ? (
            <div className={`${base}__empty`} role="status">
              {resolvedEmptyMessage}
            </div>
          ) : (
            <>
              {filteredItems.map((item, i) =>
                renderItem(item, selected, setSelected, panelId, i)
              )}
  ...
  const hasSourceSel = [...sourceSelected].some(
    v => itemMap.has(v) && !targetSet.has(v)
  );
  const hasTargetSel = [...targetSelected].some(
    v => itemMap.has(v) && targetSet.has(v)
  );
  ...
  const pickerEl = (
    ...
    >
      ...
      {renderPanel(
        ...
        sourceSelected,
        setSourceSelected,
        resolvedSourceHeader,
        sourceFilter,
        setSourceFilter,
        "source",
        sourceReorder ? moveSourceUp : undefined,
        sourceReorder ? moveSourceDown : undefined
      )}

      <div
        className={`${base}__controls`}
        role="toolbar"
        aria-label="Item picker controls"
      >
        <Button
          buttonType="outlined"
          severity="secondary"
          size="sm"
          rounded
          ariaLabel={locale.itemPicker.moveAllToTarget}
          tooltip={locale.itemPicker.moveAllToTarget}
          disabled={disabled || !hasMovableSource}
          onClick={moveAllToTarget}
          iconLeft={iconSvg(DOUBLE_ARROW_RIGHT_PATH)}
        />
        <Button
          buttonType="outlined"
          severity="primary"
          size="sm"
          rounded
          ariaLabel={locale.itemPicker.moveToTarget}
          tooltip={locale.itemPicker.moveToTarget}
          disabled={disabled || !hasSourceSel}
          onClick={moveToTarget}
          iconLeft={iconSvg(CHEVRON_RIGHT_PATH)}
        />
        <Button
          buttonType="outlined"
          severity="primary"
          size="sm"
          rounded
          ariaLabel={locale.itemPicker.moveToSource}
          tooltip={locale.itemPicker.moveToSource}
          disabled={disabled || !hasTargetSel}
          onClick={moveToSource}
          iconLeft={iconSvg(CHEVRON_LEFT_PATH)}
        />
        <Button
          buttonType="outlined"
          severity="secondary"
          size="sm"
          rounded
          ariaLabel={locale.itemPicker.moveAllToSource}
          tooltip={locale.itemPicker.moveAllToSource}
          disabled={disabled || !hasMovableTarget}
          onClick={moveAllToSource}
          iconLeft={iconSvg(DOUBLE_ARROW_LEFT_PATH)}
        />
      </div>
      ...
      {renderPanel(
        ...
        targetSelected,
        setTargetSelected,
        resolvedTargetHeader,
        targetFilter,
        setTargetFilter,
        "target",
        targetReorder ? moveTargetUp : undefined,
        targetReorder ? moveTargetDown : undefined
      )}
    </div>
  );
