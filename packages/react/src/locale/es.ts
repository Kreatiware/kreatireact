import type { KreatiLocale } from "./types";

export const es: KreatiLocale = {
  select: {
    filterPlaceholder: "Buscar...",
    emptyMessage: "Sin resultados",
    clearLabel: "Limpiar seleccion",
  },
  multiSelect: {
    filterPlaceholder: "Buscar...",
    emptyMessage: "Sin resultados",
    clearLabel: "Limpiar todo",
    selectAll: "Seleccionar todo",
    selectedCount: "{count} seleccionados",
  },
  calendar: {
    previousMonth: "Mes anterior",
    nextMonth: "Mes siguiente",
    previousYear: "Anio anterior",
    nextYear: "Anio siguiente",
    calendarLabel: "Calendario",
    today: "Hoy",
    clear: "Limpiar",
    hour: "Horas",
    hourShort: "Hrs",
    minute: "Minutos",
    minuteShort: "Min",
    second: "Segundos",
    secondShort: "Seg",
    presetToday: "Hoy",
    presetYesterday: "Ayer",
    presetLast7Days: "Ultimos 7 dias",
    presetLast14Days: "Ultimos 14 dias",
    presetLast30Days: "Ultimos 30 dias",
    presetThisWeek: "Esta semana",
    presetThisMonth: "Este mes",
    presetLastMonth: "Mes anterior",
    presetThisYear: "Este anio",
  },
  common: {
    loading: "Cargando...",
    close: "Cerrar",
    search: "Buscar",
    filterOptions: "Filtrar opciones",
    increment: "Incrementar",
    decrement: "Decrementar",
    carousel: "Carrusel",
    previous: "Anterior",
    next: "Siguiente",
    page: "Pagina",
    copy: "Copiar",
    copied: "Copiado",
    code: "Codigo",
    showPassword: "Mostrar contrasena",
    hidePassword: "Ocultar contrasena",
    selectAll: "Seleccionar todo",
  },
  slider: {
    rangeMin: "minimo",
    rangeMax: "maximo",
  },
  list: {
    filterPlaceholder: "Buscar...",
    emptyMessage: "Sin resultados",
  },
  dialog: {
    accept: "Aceptar",
    reject: "Cancelar",
    close: "Cerrar",
    confirmation: "Confirmacion",
    maximize: "Maximizar",
    restore: "Restaurar",
  },
  message: {
    close: "Cerrar mensaje",
  },
  toast: {
    close: "Cerrar notificacion",
  },
  dial: {
    ariaLabel: "Dial",
  },
  emptyState: {
    title: "Sin datos",
    description: "No hay elementos para mostrar.",
  },
  textEditor: {
    toolbar: "Formato",
    bold: "Negrita (Ctrl+B)",
    italic: "Cursiva (Ctrl+I)",
    underline: "Subrayado (Ctrl+U)",
    strikethrough: "Tachado",
    unorderedList: "Lista con vinetas",
    orderedList: "Lista numerada",
    link: "Enlace (Ctrl+K)",
    code: "Codigo",
    blockquote: "Cita",
    undo: "Deshacer (Ctrl+Z)",
    redo: "Rehacer (Ctrl+Y)",
    textStyle: "Estilo de texto",
    fontSize: "Tamano de fuente",
    fontFamily: "Familia de fuente",
    textColor: "Color de texto",
    bgColor: "Color de resaltado",
    paragraph: "Parrafo",
    heading1: "Titulo 1",
    heading2: "Titulo 2",
    heading3: "Titulo 3",
    insertLink: "Insertar",
    removeLink: "Eliminar",
    linkLabel: "Etiqueta",
    linkLabelPlaceholder: "Texto visible (opcional)",
  },
  rating: {
    ariaLabel: "Calificacion",
    clear: "Limpiar calificacion",
  },
  colorPicker: {
    ariaLabel: "Selector de color",
    saturation: "Saturacion y brillo",
    hue: "Tono",
    opacity: "Opacidad",
    switchFormat: "Cambiar formato de color",
  },
  fileUpload: {
    clickToUpload: "Haz clic para subir",
    dragAndDrop: "o arrastra y suelta",
    maxSize: "Max",
    removeFile: "Eliminar archivo",
  },
  pagination: {
    ariaLabel: "Paginacion",
    first: "Primera pagina",
    previous: "Pagina anterior",
    next: "Pagina siguiente",
    last: "Ultima pagina",
    page: "Pagina",
  },
  itemPicker: {
    sourceHeader: "Disponibles",
    targetHeader: "Seleccionados",
    moveAllToTarget: "Mover todos al destino",
    moveToTarget: "Mover al destino",
    moveToSource: "Mover al origen",
    moveAllToSource: "Mover todos al origen",
    filterPlaceholder: "Buscar...",
    emptyMessage: "Sin elementos",
    itemCount: "{count} elementos",
    moveUp: "Subir",
    moveDown: "Bajar",
  },
  dataTable: {
    ariaLabel: "Tabla de datos",
    emptyMessage: "Sin datos disponibles",
    filterPlaceholder: "Filtrar...",
    sortAscending: "Ordenar ascendente",
    sortDescending: "Ordenar descendente",
    selectAll: "Seleccionar todas las filas",
    selectRow: "Seleccionar fila",
    edit: "Editar",
    delete: "Eliminar",
    view: "Ver",
    copy: "Copiar",
    print: "Imprimir",
    rowsPerPage: "Filas",
    pageInfo: "{start}-{end} de {total}",
    exportCSV: "Exportar CSV",
    reorderRow: "Reordenar fila",
    reorderColumn: "Reordenar columna",
  },
  avatarGroup: {
    groupLabel: "Grupo de {count} avatares",
    overflowLabel: "{count} mas",
  },
  kanban: {
    boardLabel: "Tablero Kanban",
    cardLabel: "Tarjeta",
    cardCount: "tarjetas",
    addCard: "Agregar tarjeta",
    addCardPlaceholder: "Titulo de la tarjeta...",
    grabbed:
      "{title} seleccionada. Usa las flechas para mover, Enter para soltar, Escape para cancelar",
    dropped: "{title} soltada",
    cancelled: "Movimiento cancelado",
    movedToColumn: "{title} movida a {column}",
    movedPosition: "{title} movida a posicion {position}",
    cardDetail: "Detalles de la tarjeta",
    createCard: "Crear tarjeta",
    editCard: "Editar tarjeta",
    title: "Titulo",
    description: "Descripcion",
    priority: "Prioridad",
    dueDate: "Fecha limite",
    save: "Guardar",
    cancel: "Cancelar",
    create: "Crear",
    deleteCard: "Eliminar",
    confirmDelete: "Estas seguro de que deseas eliminar esta tarjeta?",
    priorityLow: "Baja",
    priorityMedium: "Media",
    priorityHigh: "Alta",
    priorityCritical: "Critica",
    noPriority: "Ninguna",
    tags: "Etiquetas",
    tagsPlaceholder: "Seleccionar etiquetas...",
    assignees: "Asignados",
    assigneesPlaceholder: "Seleccionar asignados...",
    checklist: "Lista de tareas",
    checklistAdd: "Agregar tarea...",
    descriptionPlaceholder: "Haz clic para agregar una descripcion...",
    cover: "Imagen de portada",
    coverPlaceholder: "https://...",
    duplicate: "Duplicar",
    moveToNext: "Mover a siguiente columna",
    addColumn: "Agregar columna",
    renameColumn: "Renombrar",
    deleteColumn: "Eliminar columna",
    moveColumnLeft: "Mover a la izquierda",
    moveColumnRight: "Mover a la derecha",
    confirmDeleteColumn:
      "Estas seguro? Las tarjetas de esta columna se eliminaran.",
    newColumnTitle: "Nueva columna",
    editColumn: "Editar columna",
    columnSeverity: "Color de acento",
    columnMaxCards: "Limite de tarjetas (WIP)",
    severityNone: "Ninguno",
    collapse: "Colapsar",
    expand: "Expandir",
  },
  chart: {
    resetZoom: "Restablecer zoom",
    exportPng: "Exportar como PNG",
    exportSvg: "Exportar como SVG",
    exportCsv: "Exportar como CSV",
    exportJsonTable: "Exportar como JSON (tabla)",
    exportJsonSeries: "Exportar como JSON (series)",
    menuLabel: "Menu del grafico",
    filterPlaceholder: "Filtrar series...",
    filterLabel: "Series",
    exportLabel: "Exportar",
    tooltipSingle: "Individual",
    tooltipShared: "Compartido",
    othersLabel: "Otros",
  },
  gantt: {
    ganttLabel: "Diagrama de Gantt",
    chartLabel: "Diagrama de Gantt",
    viewDay: "Dia",
    viewWeek: "Semana",
    viewMonth: "Mes",
    today: "Hoy",
    start: "Inicio",
    end: "Fin",
    progress: "Progreso",
    duration: "Duracion",
    assignees: "Asignados",
    milestone: "Hito",
    summary: "Resumen",
    expand: "Expandir",
    collapse: "Contraer",
    edit: "Editar",
    delete: "Eliminar",
    editTask: "Editar tarea",
    deleteConfirm: "Estas seguro de que deseas eliminar esta tarea?",
    save: "Guardar",
    cancel: "Cancelar",
    title: "Titulo",
    severity: "Severidad",
    none: "Ninguno",
    date: "Fecha",
    filter: "Filtrar tareas...",
    export: "Exportar",
    criticalPath: "Ruta critica",
    baseline: "Linea base",
    autoSchedule: "Auto-programar",
    addTask: "Agregar tarea",
    newTask: "Nueva tarea",
    newGroup: "Nuevo grupo",
    resetZoom: "Restablecer zoom",
    undo: "Deshacer",
    redo: "Rehacer",
    addDependency: "Agregar dependencia...",
    finishToStart: "Fin a Inicio",
    startToStart: "Inicio a Inicio",
    finishToFinish: "Fin a Fin",
    startToFinish: "Inicio a Fin",
    expandTable: "Expandir tabla",
    collapseTable: "Contraer tabla",
    dependsOn: "Depende de",
    requiredBy: "Requerido por",
  },
};
